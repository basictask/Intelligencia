...
\textbf{Mivel a MC epizódonként vesz mintát a hozamokból, az értékfüggvények csak sok epizód hozamának átlagolásaként számítódhatnak ki}.\par\smallskip
...
\begin{columns}
\begin{column}{.5\textwidth}
\only<1>{A $V_{\pi}(s)$ \textbf{állapot-érték függvény egy állapotonkénti vektor, amely minden állapot esetén számon tartja az adott $s$ állapotból indulva összegyűjtött jutalmak várható értékét} ha az ügynök egy adott $\pi$ politika szerint cselekszik.}
\only<2>{A $Q_{\pi}(s,a)$ \textbf{állapot-cselekvés minőség függvény egy mátrix, amely az állapot-cselekvés párokhoz tartozóan tárolja el, hogy egy adott $s$ állapotból $a$ cselekvést végrehajtva mennyi a jutalom várható értéke} ha az ügynök egy adott $\pi$ politika szerint cselekszik.}
\par\smallskip
