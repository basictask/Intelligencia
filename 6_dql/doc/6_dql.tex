...
\For{$i = 0 \rightarrow max_i$}{
	...
	\For{$c = 0 \rightarrow max_c$}{
		...
		Gradiens ereszkedés végrehajtása $\left( r + \gamma \underset{a'}{max}Q_\theta(s',a') - Q_\theta(sa) \right)^2$ hibán
	}		
}
\end{algorithm}
...
\For{$i = 0 \rightarrow max_i$}{
	...
	\For{$c = 0 \rightarrow max_c$}{
		...
		Gradiens ereszkedés végrehajtása a $(Q^*(s_t,a_t) - Q_\theta(s_t,a_t))^2$ hibán\;
